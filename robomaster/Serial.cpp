...
bool Serial::sendXYZ(int fd, double * xyz){
    ...
    data_ptr[0] = (short)xyz[0];  // x
    data_ptr[1] = (short)xyz[1];  // y
    data_ptr[2] = (short)xyz[2];  // z
    if (8 == write(fd, send_bytes, 8))         // Send data
        return true;
    return false;
}

bool Serial::sendData(int fd, char * data, int size, Serial::DATATYPE data_tpye){
